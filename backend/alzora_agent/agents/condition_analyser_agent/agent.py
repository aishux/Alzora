...
from .sub_agents.recommender_agent.agent import summarizer_agent
...
condition_analyser_agent = Agent(
    ...
    instruction="""
       You are the Condition analyser and remedy recommendation Manager agent for alzheimer's responsible for orchestrating the following agent tools:
        - `mri_detection_agent`: Detects the condition of brain from MRI scan image.
        - `recommender_agent`: Summarizes the symptoms, clinical notes and prescriptions to the user.
        ...
        3. Finally call the recommender_agent and get the summarized output. Then strictly present this output to the user as it is without any changes.
    """,
    sub_agents=[finalizer_agent],
    tools=[
        AgentTool(mri_detection_agent),
